...
from kerasdemo.voice.utils.estnoise_ms import stft, estnoisem, bessel
import matplotlib.pyplot as plt
import numpy as np
import scipy.io.wavfile as wav
# from voice.utils.estnoise_ms import *
...
#         gain = (math.gamma(1.5) * np.sqrt(V)) / aPosterioriSNR_frame * \
#             np.exp(-1 * V / 2) * ((1 + V) * bessel(0, V / 2) + V * bessel(1, V / 2))
# 
...
#     path_wav = os.path.join(os.getcwd(), "datasets/SI1265_FJWB0_2.wav")
    path_wav = os.path.join(os.getcwd(), "datasets/unit1.WAV")
    (sr, signal) = wav.read(path_wav)

    vad = VAD(signal, sr, nFFT=512, win_length=0.025, hop_length=0.01, theshold=0.7)
#     last =0
#     start=0
#     end=0
#     index = 0
#     for i, val in enumerate(signal):
#         val = 1 if abs(val) > 900 else 0
#         if val ==0:
#             if start > 0:
#                 end = i-1
#         elif val != 0:
...
#             print("%s,%s,s=%s,e=%s"%(i,last,start,end))
#             start = 0
#             end = 0
#         last = val
